...
-- 1. WHENEVER THERE IS AN INSERTION OR DELETION ON STUDENT TABLE,  
--    THE CURRENTLY INSERTED OR DELETED ROW SHOULD BE PRINTED AS IT IS  
--    ON THE OUTPUT CONSOLE WINDOW.  
...
1. Whenever a new employee is inserted into tbl_employee,  
   a record should be added to tbl_employee_audit like:  
   "Employee name <emp_name> has been added at <current_time>"

2. Whenever an employee is deleted from tbl_employee,  
   a record should be added to tbl_employee_audit like:  
